...
import { Waves, ScanLine, ArrowLeftRight, ArrowUpRight, RotateCcw, Undo2, Lightbulb, Crown, ArrowRight, CircleHelp } from 'lucide-react';
...
const GAMES = [
  { id: 'ripple' as Game, name: '파문', en: 'RIPPLE', icon: Waves, desc: '한 수가 움직이는 세계', lead: '하나를 놓고,\n주변의 가능성을 움직이세요.', rules: [['돌 하나 놓기','흑부터 번갈아 빈칸에 돌을 놓습니다.'],['주변에 파문 만들기','상하좌우에 붙은 돌이 한 칸 밀립니다. 다른 돌로 막히면 멈추고, 판 밖이면 사라집니다.'],['네 개를 연결하기','모든 이동 후 가로·세로·대각선에 내 돌 네 개를 연결하면 승리합니다.']], thought: '상대의 돌도 내 돌을 지키는 받침이 될 수 있습니다.' },
  { id: 'erosion' as Game, name: '침식', en: 'EROSION', icon: ScanLine, desc: '돌아갈 수 없는 선택', lead: '자유롭게 움직일수록,\n세계는 작아집니다.', rules: [['내 말 선택하기','각자 세 말로 시작합니다. 흑부터 자기 말 하나를 선택합니다.'],['길을 지우며 이동하기','상하좌우로 원하는 만큼 이동합니다. 말과 구멍을 넘을 수 없고, 출발한 칸은 영구히 사라집니다.'],['마지막까지 살아남기','자기 차례에 어느 말도 이동할 수 없으면 패배합니다.']], thought: '상대를 가두기 전에, 나에게 남을 빈칸을 세어보세요.' },
  { id: 'legacy' as Game, name: '유산', en: 'LEGACY', icon: ArrowLeftRight, desc: '상대에게 남기는 가능성', lead: '지금 쓴 능력이,\n상대의 다음 수가 됩니다.', rules: [['이동 패 고르기','자기 차례에는 패 세 장이 있습니다. 패와 말을 고르면 가능한 목적지가 표시됩니다.'],['능력을 넘겨주기','패의 모양대로 이동한 뒤 사용한 패를 상대에게 넘깁니다. 두 칸 이동은 중간 말을 뛰어넘습니다.'],['중심 말을 잡기','왕관이 있는 상대 말을 잡으면 승리합니다. 상대가 움직일 수 없어도 승리합니다.']], thought: '내 왕을 향하는 공격 패를 상대에게 넘기고 있지는 않나요?' },
];
...
export default function Home() {
  ...
  const caption=notice|| (hint?`추천 수: ${describeMove(state,hint)}. 강조된 칸을 선택하세요.`:thinking?'백이 다음 수를 생각하고 있어요.':state.winner!==null?state.reason:game==='ripple'?'빈칸에 올려놓으면 파문을 미리 볼 수 있어요.':selected!==null?'밝게 표시된 칸으로 이동할 수 있어요.':game==='legacy'?'아래에서 패를 고른 뒤, 움직일 말을 선택하세요.':'내 말을 선택하면 갈 수 있는 길이 나타나요.');
  ...
  return <main className="studio">
    <header className="masthead"><a href="/" className="wordmark"><span className="brand-mark">∴</span>한 수<span className="brand-en">HANSU</span></a><div className="edition"><span className="live-dot"/> ABSTRACT GAMES · VOL. 01</div></header>
    <section className="intro"><div><p className="eyebrow">작은 규칙, 깊은 선택</p><h1>다음 한 수는, 당신에게.</h1></div><p className="intro-note">세 가지 세계. 서로 다른 생각의 방식.<br/>한 판씩, 천천히 발견해보세요.</p></section>
    <Tabs value={game} onValueChange={v=>{setGame(v as Game);clear();}}><TabsList className="game-tabs" aria-label="게임 선택">{GAMES.map((g,i)=><TabsTrigger key={g.id} value={g.id} className="game-tab"><g.icon/><div><span className="tab-title">{g.name}<small>{g.en}</small></span><span className="tab-desc">{g.desc}</span></div><span className="tab-number">0{i+1}</span></TabsTrigger>)}</TabsList></Tabs>
    <div className="match-controls"><Tabs value={mode} onValueChange={v=>{setMode(v as string);clear();}}><TabsList aria-label="대전 방식" className="mode-tabs"><TabsTrigger value="ai">AI와 두기</TabsTrigger><TabsTrigger value="local">둘이 두기</TabsTrigger></TabsList></Tabs><div className="settings">{mode==='ai'&&<><span className="you-play">나는 흑 · AI는 백</span><Select value={level} onValueChange={v=>setLevel(v as 'easy'|'normal')}><SelectTrigger aria-label="AI 난이도"><SelectValue>{level==='easy'?'가볍게':'신중하게'}</SelectValue></SelectTrigger><SelectContent><SelectItem value="easy">가볍게</SelectItem><SelectItem value="normal">신중하게</SelectItem></SelectContent></Select></>}<Button variant="ghost" className="new-game" onClick={reset}><RotateCcw size={15}/>새 대국</Button></div></div>
    <section className="play-layout"><div className="play-surface"><div className="table-top"><span role="status" aria-live="polite" data-testid="turn-status"><span className={`tiny-stone ${state.turn===1?'black':'white'}`}/>{state.winner!==null?state.winner==='draw'?'무승부':`${colorName(state.winner)} 승리`:thinking?'백이 생각 중':`${colorName(state.turn)}의 차례`}{thinking&&<span className="thinking-dots">···</span>}</span><span className="table-meta" data-testid="ply">{String(state.ply).padStart(2,'0')}수 · {state.size} × {state.size}</span></div>
    {game==='legacy'&&<div className="opponent-hand"><span>{colorName(state.turn===1?2:1)}의 패</span>{state.hands[(state.turn===1?2:1)-1].map(c=><span className="waiting-card" key={c}>{CARDS[c].name}</span>)}<span className="pass-note">사용한 패가 이쪽으로 <ArrowRight size={12}/></span></div>}
    <div className="board-frame"><div className="board" role="group" aria-label={`${info.name} 게임판`} style={{gridTemplateColumns:`repeat(${state.size},1fr)`}} onPointerLeave={()=>setHover(null)}>{display.map((v,i)=>{
      ...
      return <button key={i} data-cell={i} data-piece={state.board[i]} aria-label={`${coord(i,state.size)} ${state.board[i]===-1?'사라진 칸':state.board[i]===0?'빈칸':colorName(owner(state.board[i]))+(state.board[i]>=3?' 중심 말':' 돌')}${available?' 이동 가능':''}`} aria-disabled={thinking||state.winner!==null||v===-1} className={`cell ${v===-1?'void':''} ${selected===i?'selected':''} ${available?'available':''} ${hinted?'hinted':''} ${preview&&v!==state.board[i]?'preview':''} ${winCells.includes(i)?'winning':''}`} onPointerEnter={()=>setHover(i)} onFocus={()=>setHover(i)} onBlur={()=>setHover(null)} onClick={()=>clickCell(i)} onKeyDown={e=>{const offsets:Record<string,number>={ArrowUp:-state.size,ArrowDown:state.size,ArrowLeft:-1,ArrowRight:1};if(e.key in offsets){e.preventDefault();const next=i+offsets[e.key];if(next>=0&&next<state.board.length)document.querySelector<HTMLButtonElement>(`[data-cell="${next}"]`)?.focus();}if(e.key==='Escape'){setSelected(null);setHint(null);}}}>
        {v>0&&<span className={`stone ${own===1?'black':'white'}`}>{v>=3&&<Crown size={20} strokeWidth={1.5}/>}<span className="sr-only">{colorName(own)}</span></span>}
        {available&&v===0&&<span className="move-dot"/>}{v===-1&&<span className="void-cross">×</span>}{state.last?.to===i&&!preview&&<span className="last-mark"/>}<span className="cell-coord">{coord(i,state.size)}</span>
      </button>;
    })}</div></div>
    {game==='legacy'&&<div className="hand"><div className="hand-title">{colorName(state.turn)}의 이동 패 <span>화면 방향 그대로 이동합니다</span></div><div className="cards">{state.hands[state.turn-1].map(c=><button key={c} aria-label={`${CARDS[c].name} 패`} aria-pressed={activeCard===c} disabled={thinking||state.winner!==null} className={`move-card ${activeCard===c?'active':''}`} onClick={()=>{setCard(c);setHint(null);}}><CardPattern card={c} turn={state.turn}/><span>{CARDS[c].name}{activeCard===c&&<small>선택됨</small>}</span></button>)}</div></div>}
    <p className="board-caption" aria-live="polite">{caption}</p>
    {state.winner!==null&&<div className="result" data-testid="result"><strong>{state.winner==='draw'?'서로의 한 수가 맞섰습니다.':`${colorName(state.winner)}의 한 수가 닿았습니다.`}</strong><Button onClick={reset}>한 판 더 <ArrowUpRight size={16}/></Button></div>}
    ...
    </div><aside className="guide-panel"><div className="guide-heading"><span className="eyebrow">HOW TO PLAY</span><ArrowUpRight size={20}/></div><h2>{info.name}</h2><p className="guide-lead">{info.lead.split('\n').map((text,i)=><span key={text}>{i>0&&<br/>}{text}</span>)}</p><ol className="rules-list">{info.rules.map(([title,body])=><li key={title}><b>{title}</b><p>{body}</p></li>)}</ol><div className="thought"><span><Lightbulb size={13}/> 생각의 실마리</span><p>{info.thought}</p></div><details className="more-rules"><summary><CircleHelp size={14}/>세부 규칙</summary><p>{game==='ripple'?'돌의 밀림은 동시에 처리됩니다. 두 색이 함께 네 개를 연결하거나 판이 가득 차면 무승부입니다. ':game==='legacy'?'패 다섯 장을 공유합니다. 시작할 때 흑이 세 장, 백이 두 장을 가지며, 자기 차례에는 항상 세 장이 됩니다. 패의 방향은 백에게 180° 회전합니다. ':''}{game!=='erosion'?'차례와 배치(유산은 패 포함)가 세 번 반복되거나 160수가 지나면 무승부입니다.':'출발 칸만 사라집니다. 지나간 칸과 도착 칸은 남습니다.'} 게임을 바꾸어도 현재 판은 유지됩니다. 새로고침하면 초기화됩니다.</p></details><div className="move-log"><span className="eyebrow">최근 수</span>{history.length===1?<p className="log-empty">첫 수를 기다리고 있어요.</p>:history.slice(Math.max(1,history.length-3)).map(s=><p key={s.ply}><small>{String(s.ply).padStart(2,'0')}</small>{describeMove(history[s.ply-1],s.last!)}</p>)}</div></aside></section>
    <footer><span>HANSU — THE ABSTRACT TRILOGY</span><span>실험판 0.1 · 서두르지 않아도 좋은 게임.</span></footer>
  </main>;
}
