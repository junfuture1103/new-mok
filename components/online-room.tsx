...
export default function OnlineRoom() {
  ...
  function persist(next: Entry) {
    entry.current = next;
    try { sessionStorage.setItem(entryKey(next.code), JSON.stringify(next)); sessionStorage.setItem(lastKey, next.code); localStorage.setItem('hansu:nickname', next.nickname); }
  ...
  async function enter(kind: 'create' | 'join', restore?: Entry) {
    ...
    try {
      ...
      next = restore ?? (pending && pending.kind === kind && pending.nickname === name && pending.game === game && (kind === 'create' || pending.code === requested) ? pending : { kind, code: kind === 'create' ? randomCode() : requested, token: randomToken(), nickname: name, game });
    ...
    try {
      ...
      accept(result, next); setConfirmation(null); history.replaceState(null, '', roomUrl(next));
    } catch (e) {
  ...
  useEffect(() => {
    ...
    try {
      setNickname(localStorage.getItem('hansu:nickname') || '');
      const previous = requested || sessionStorage.getItem(lastKey);
      if (previous && ROOM_CODE.test(previous)) {
        const saved = JSON.parse(sessionStorage.getItem(entryKey(previous)) || 'null') as Entry | null;
        if (saved && saved.code === previous && /^[a-f0-9]{64}$/.test(saved.token) && ['create', 'join'].includes(saved.kind) && Object.hasOwn(GAME_NAMES, saved.game)) {
  ...
  useEffect(() => {
    ...
    async function poll() {
      ...
      inFlight = true;
      try { const next = await api(expected); if (!stopped) { accept(next, expected); failures = 0; } }
      catch (e) {
        if (!stopped) {
          setConnected(false); failures++;
          if ([401, 403, 404, 410].includes((e as ApiError).status)) { setError((e as Error).message); stopped = true; }
        }
      } finally {
        inFlight = false;
  ...
  function exitRoom() {
    ...
    if (previous) { try { sessionStorage.removeItem(entryKey(previous.code)); sessionStorage.removeItem(lastKey); } catch { /* Optional storage. */ } }
  ...
  const status = !room ? '' : !connected ? '연결 확인 중 · 자동으로 다시 연결합니다' : room.phase === 'waiting' ? '친구를 기다리고 있어요' : room.phase === 'closed' ? '닫힌 방' : room.phase === 'finished' ? room.state.winner === 'draw' ? '무승부' : `${room.players[Number(room.state.winner) - 1]?.nickname} 님 승리` : busy ? '착수를 전송하고 있어요' : room.state.turn === room.you ? '내 차례입니다' : `${opponent?.nickname} 님의 차례`;
  return <main className="studio online-studio" data-game={room?.game ?? game} data-ready={ready}>
    ...
    {!room ? <section className="online-lobby" aria-busy={busy}>
      ...
      <fieldset className="lobby-form" disabled={!ready || busy}>
        <label htmlFor="online-nickname">내 닉네임</label><input id="online-nickname" value={nickname} maxLength={24} placeholder="1~12자" autoComplete="nickname" onChange={e => setNickname(e.target.value)} />
      ...
      <div className="play-surface">
        ...
        {confirmation && <div className="online-confirm" role="alertdialog" aria-labelledby="confirm-title"><p id="confirm-title">{confirmation === 'resign' ? '기권하고 이 대국을 마칠까요?' : room.phase === 'playing' ? '지금 나가면 상대의 승리로 대국이 끝나요. 나갈까요?' : '방을 닫고 나갈까요?'}</p><Button disabled={busy || !connected} onClick={() => { void action(confirmation); }}>{confirmation === 'resign' ? '기권하기' : '나가기'}</Button><Button variant="outline" disabled={busy} onClick={() => setConfirmation(null)}>계속 머물기</Button>{!connected && <Button variant="ghost" disabled={busy} onClick={exitRoom}>대기실로 돌아가기</Button>}</div>}
      </div>
