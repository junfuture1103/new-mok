...
export default defineConfig({
  ...
  webServer:[{command:'node scripts/serve.mjs',url:baseURL,reuseExistingServer:true},
    ...(process.env.NEXT_PUBLIC_ROOM_API?.startsWith('http://127.0.0.1:4191') ? [{command:'node --experimental-strip-types scripts/serve-api.ts',url:'http://127.0.0.1:4191/api/health',reuseExistingServer:true}] : []),
  ],
});
